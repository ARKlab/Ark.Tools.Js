...
const excludesPrimaryKey = R.curry(
  (tableData: any, modalData: any, pks: any, errMsg: any): any =>
    tableData.filter(checkPrimaryKey({ row: modalData.row, pks })).length > 0
      ? Left([errMsg])
      : Right(modalData)
);

const primaryKeyValuesMatch = (modalData: any, pks: any, errMsg: any): any =>
  R.all((x: any) => R.equals(modalData.row[x], modalData.orig[x]), pks)
    ? Right(modalData)
    : Left([errMsg]);

const onlyNonPrimaryAltered = (modalData: any, pks: any, errMsg: any) =>
  R.pipe(
...
const getUpdater = reader(([pks, errMsg, modalData, tableData]: any) =>
  R.pipe(
    isUpdate,
...
const getAdder = reader(([pks, errMsg, modalData, tableData]: any) =>
  R.pipe(
    isAdd,
...
export const commitChangesHelper = getModifiers.run;
